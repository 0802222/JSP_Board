...
<%
    ...
    try (Connection conn = DatabaseConfig.getConnection()) {
        // 조회수 증가
        try (PreparedStatement updateStmt = conn.prepareStatement("UPDATE board SET views = views + 1 WHERE no = ?")) {
            updateStmt.setInt(1, Integer.parseInt(postNo));
            updateStmt.executeUpdate();
        }

        // 게시글 내용 가져오기
        try (PreparedStatement selectStmt = conn.prepareStatement("SELECT * FROM board WHERE no = ?")) {
            selectStmt.setInt(1, Integer.parseInt(postNo));
...
<%
        ...
        e.printStackTrace();
        out.println("<script>alert('오류 발생: " + e.getMessage() + "'); location.href='board.jsp';</script>");
    }
%>
